...
function consumer_login_details($LOGIN_EMAIL, $LOGIN_PASSWORD, $con)
{
$result = mysqli_query($con,"SELECT CONSUMER_ID FROM CONSUMER WHERE binary CONSUMER_EMAIL='$LOGIN_EMAIL' AND PASSWORD = '$LOGIN_PASSWORD'");
...
function get_user_details($LOGIN_USERNAME, $LOGIN_PASSWORD, $con)
{
	...
	else
	{
		...
		$profile_picture_location = "https://www.vasitars.com/old/prajjawala/".profile_picture_location($LOGIN_USERNAME);
...
function get_consumer_details($LOGIN_EMAIL, $LOGIN_PASSWORD, $con)
{
	$result = mysqli_query($con,"SELECT * FROM CONSUMER WHERE binary CONSUMER_EMAIL='$LOGIN_EMAIL' AND PASSWORD = '$LOGIN_PASSWORD'");
	if(!$result || mysqli_num_rows($result) == 0)
	{
		echo "Unable to login now. Please try again.";
	}
	else
	{
		...
		//$low_name = strtolower($RETAILER_NAME);
		$DATA_RETURNED = array('NAME' => $NAME, 'CONSUMER_ID' => $CONSUMER_ID, 'EMAIL' => $EMAIL, 'DOB' => $DOB, 'PHONE_NUMBER' => $PHONE_NUMBER, 'AADHAAR_NUMBER' => $AADHAAR_NUMBER, 'CITY' => $CITY, 'STATE' => $STATE, 'COMPLETE_ADDRESS' => $COMPLETE_ADDRESS, 'PIN => $PIN', 'RETAILER_ID' => $RETAILER_ID, 'USERNAME' => $CONSUMER_LOGIN_USERNAME);
		//$profile_picture_location = profile_picture_location($LOGIN_USERNAME);
		return $DATA_RETURNED;
	}
}
